...
const TodosList = () => {
  ...
  const handleAddNewActivity = () => {
    ...
    let newId: number;
    if (typeof todos.length === "number") {
      newId = todos.length ? (todos[todos.length - 1].id as number) + 1 : 1;

      setTodos([...todos, { id: newId, ...newActivity }]);
      CustomLocalStorage.set("todos", [
        ...todos,
        { id: newId, ...newActivity },
      ]);
    }

    setShowModal(false);
    document.body.style.overflow = "auto";
    setNewActivity({ title: "", completed: false, userId: 1 });
  };
  const handleCompleteActivity = (id: number) => {
    ...
    const updatedList: Array<Record<string, string | number | boolean>> =
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
              <div className="bg-background p-6 rounded-lg shadow-lg flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select
                    value={newActivity.completed as unknown as string}
                    onChange={(e) =>
                      setNewActivity({
                        ...newActivity,
                  ...
                  <div className="grid grid-cols-[1fr_auto] w-full justify-center items-center gap-4">
                    ...
                    {todo.completed ? (
                      <button
                        ...
                        onClick={() =>
                          handleCompleteActivity(todo.id as number)
                        }
