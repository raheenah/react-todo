...
const Footer = () => {
  ...
  return (
    <div className='bg-background pb-8 px-8'>
      {" "}
      <div className='py-4 flex flex-col gap-2 lg:max-w-[80%] w-full justify-center items-center mx-auto  shadow-custom-todo rounded-lg px-2'>
        <h3 className='text-text-primary font-bold text-lg'>
          Say Hi… or Stay Shy
        </h3>
        <div className='flex md:w-[60%] w-full justify-between items-center mt-6  '>
          <ul className='flex gap-4 '>
            {FooterData.map((item) => (
              <li key={item.id} className='flex'>
                {/* <div className='bg-blue-300 flex items-center justify-center relative w-full h-full'> */}
                <NavLink to={item.link} className='text-button-bg hover:text-button-hover text-lg '>
